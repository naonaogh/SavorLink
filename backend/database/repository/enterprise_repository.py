...
class EnterpriseRepository:
    async def list(self, session: AsyncSession, *, limit: int = 50, offset: int = 0) -> Sequence[Enterprise]:
        stmt = select(Enterprise).order_by(Enterprise.id.desc()).limit(limit).offset(offset)
