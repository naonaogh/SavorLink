...
from backend.database.schemas.cart import CartItemCreate, CartItemUpdate
from backend.database.repository.cart_repository import CartRepository
...
class CartService:
    ...
    async def get_cart(self, session: AsyncSession, *, user_id: int):
        cart = await self.cart_repo.get_by_user_id(session, user_id)
        if not cart:
            # В MVP корзина создаётся при первом добавлении, но endpoint /cart/me удобнее всегда возвращать корзину
