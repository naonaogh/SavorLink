...
class ChatService:
    ...
    async def list_chats(self, session: AsyncSession, *, user_id: int, limit: int = 50, offset: int = 0) -> Sequence[Chat]:
    ...
    async def create_chat(self, session: AsyncSession, *, user_id: int, data: ChatCreate) -> Chat:
        ...
        chat = Chat(
            ...
            user_min=min(user_id, data.user2_id),
            user_max=max(user_id, data.user2_id),
        )
        return await self.repo.create(session, chat)

    async def create_message(self, session: AsyncSession, *, chat_id: int, user_id: int, data: MessageCreate) -> Message:
        chat = await self.get_chat(session, chat_id=chat_id, user_id=user_id)
        msg = Message(chat_id=chat.id, sender_id=user_id, content=data.content, is_read=False)
