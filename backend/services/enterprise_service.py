...
class EnterpriseService:
    ...
    def _populate_computed_fields(self, ent: Enterprise) -> None:
        """Пополняет объект вычисляемыми полями для схем."""
    ...
    async def link_user_to_enterprise(self, session: AsyncSession, enterprise_id: int, user_id: int) -> None:
        ...
        # Получаем пользователя с предзагрузкой предприятий
        ...
        if not user or not ent:
            raise HTTPException(status_code=404, detail="Пользователь или предприятие не найдено")
